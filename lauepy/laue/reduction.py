...
def main():
    ...
    Default_Config = {
        ...
        'NLayer': 1,
        'idxLayer': 3,
        # if is int, serve as start index. //binary name is
        # f'{binInitial}z{idxLayer[lIdxLayer[i]]}_{str(lIdxRot[i]).zfill(digitLength)}.bin{lIdxDet[i]}'
    ...
    for layer in range(NLayer):
        ...
        if generateBkg:
            if not rank:
                lBkg = reduction.median_background(initial, startIdx, bkgInitial, NRot=NRot, NDet=NDet, NLayer=1,
                                                   layerIdx=[idxLayer[layer]], digitLength=c.digitLength, end=extention,
                                                   logfile=logfile, imgshape=[1028, 512])
            else:
                lBkg = None
        ...
        if generateBin:
            for i in range(NPerCore):
                ...
                print("fName", fName)
                logfile.write(
                    f"generate binary: rank: {rank} : layer: {lIdxLayer[i]}, det: {lIdxDet[i]}, rot: {lIdxRot[i]}, "
                    f"{os.path.basename(fName)}\n")
                sys.stdout.write(
                    f"\r generate binary: rank: {rank} : layer: {lIdxLayer[i]}, det: {lIdxDet[i]}, rot: {lIdxRot[i]}, "
                    f"{os.path.basename(fName)}\n")
                sys.stdout.flush()
                try:
                    ...
                    img = np.zeros([1028, 512])
                    logfile.write(
                        f"ERROR: FILEMISSING: rank: {rank} : layer: {lIdxLayer[i]}, det: {lIdxDet[i]}, "
                        f"rot: {lIdxRot[i]}, {os.path.basename(fName)} MISSING\n")
                # img = tifffile.imread(fName)
                binFileName = f'{binInitial}z{lIdxLayer[i]}_{str(lIdxRot[i]).zfill(digitLength)}.bin{lIdxDet[i]}'
                # print("binFileName is",binFileName)
