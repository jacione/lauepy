...
def median_background(initial, startIdx, outInitial, NRot=720, NDet=2, NLayer=1, layerIdx=[0], digitLength=5,
                      end='.tif', imgshape=[512, 512], logfile=None):
    '''
    ...
    for layer in range(NLayer):
        ...
        for det in range(NDet):
            ...
            for rot in range(NRot):
                ...
                # f'{binInitial}z{lIdxLayer[i]}_{str(lIdxRot[i]).zfill(digitLength)}.bin{lIdxDet[i]}'
    ...
    for i in range(int(len(lIdxStart) / 2)):
        ...
        if vMax > baseline:
            ...
            for j in range(start, end):
                if lSubTmp[j] > (
                max(vMax * 0.1, 1)):  # if subtraction image above 10% of max intensity, take it as part of the peak.
                    lXX.append(lXTmp[j])
                    lYY.append(lYTmp[j])
...
def segmentation_numba(img, bkg, baseline=10, minNPixel=4, medianSize=3):
    ...
    return (img.shape[1] - 1 - np.array(lY)).astype(np.int32), np.array(lX).astype(np.int32), np.array(
        lIntensity).astype(np.int32), np.array(lID).astype(np.int32)
...
def reduce_image(initial, startIdx, bkgInitial, binInitial, NRot=720, NDet=2, NLayer=1, idxLayer=[0], end='.tif',
                 imgshape=[512, 512],
...
if __name__ == '__main__':
    import src.hexomap.IntBin as IntBin
