...
import src.lauepy.pflibs as pf
from src.rxlibs.xmd34 import geometry as geo
from src.rxlibs.xmd34 import lattice as latt
...
def main():
    ...
    [peak_dict, _, _] = pf.get_peaklist(filename, threshold)
    ...
    delg_gonio = pd.read_csv('delg_gonio' + str(data['H'][k]) + str(data['K'][k]) + str(data['L'][k]) + '.csv',
                             header=None,
                             delimiter=',').values
    ...
    xtal_simu = fsim.LaueSimulation(geo_ccd, Si_xtal, xtal_rmat, keV_cutoffs=energy_cutoff_keV,
    ...
    def loss_function_distance(x):
        ...
        xtalsimu = fsim.FastLaueSimulation_list(hkl_labels, geoccd, Si_xtal, xtal_rmat)
                ...
                for R0 in np.linspace(1.15, 1.25, 3):
                    ...
                    for R1 in np.linspace(1.15, 1.25, 3):
                        ...
                        for R2 in np.linspace(1.15, 1.25, 3):
                            e = so.minimize(loss_function_distance,
                                            np.array([P0, P1, P2, R0, R1, R2]),
                                            method='Nelder-Mead')
                            print(e.fun)
                            if e.fun < tolerance:
    ...
    # M = pf.extract_rlv()
    # data = pd.read_csv('/Users/pateras/PycharmProjects/pythonProject/data2BraggPeakMeasurement.csv', sep=',',
    # index_col=0)
